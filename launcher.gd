...
func _ready() -> void:
	...
	mod_ready_to_play.connect(func():
		...
		var args = ["-novr", "+vr_enable_fake_vr", "1", "-condebug", "+hlvr_main_menu_delay", "999999", "+hlvr_main_menu_delay_with_intro", "999999", "+hlvr_main_menu_delay_with_intro_and_saves", "999999", custom_launch_options.text, " -window"]
		var packed_args = PackedStringArray(args)
		OS.create_process(".\\hlvr.exe", packed_args) 
		# HUGE thanks to Ars for helping figure out how to get this working without launching through Steam!
		...
		label_info.text = "I have modified this launcher to directly launch hlvr.exe, so if it isn't
		next to that file in 'game/bin/win64', or if it gives you any trouble,close this launcher - fix
		any issues there may be - then relaunch it and try again. Thank you for your understanding,
		and I hope this works as expected. ~veeanti<3"
